...
export const Anchor: FC<AnchorProps> = ({
  ...
  if (disabled || !href) {
    // include data-[...] props for tests
    const buttonProps = pickBy(props, (_value, key) => key.startsWith('data'));
    return (
      <Button disabled {...buttonProps} {...{ className, danger, icon, loading, size, variant }}>
        {children}
      </Button>
    );
  }
  return (
    <Link
      className={buildClassName({ className, danger, icon, loading, size, variant })}
      href={href}
      {...props}
    >
